...
| ---------- | ------ | -------- | ---------------------------------------- |
| uri        | String | Yes      | The URI you want to jump to can be in the following format:Complete URIs that contain "schema"; the schemas that are currently supported are "tel", "sms", and "mailto", e.g. tel: 10086.Using web paths in the app beginning with '/'; e.g.: /about.Using page names within the app that don't start with '/'; e.g.: About.Special format. When URI value is "/", the user will be redirected to the page with the path "/", and not the start page.`101+ `supports URI with "schema" elements. Here's how you process such URIs:Find all the page filters set under the app to choose a suitable page processing request (see [Manifest](../../framework/manifest.md)).If no suitable page can handle the request, then it will use default strategy to process the request. Currently the default strategy supports schema processing such as HTTPS and internal.If the default strategy can't process the request, it will try to use an app in the system to process it.If there is no system app that can process the request, then it will discard the request.The processing logic for the default strategy:If schema is HTTP/HTTPS, it will use the internal web page to open the page.If the schema is internal (view [File organization](../../framework/file-organization.md)), it will determine the file type according to the URI file extension and then call the app in the system again to open the file. |
...
`101+` added the following method:
